...
class ClientController extends Controller
{
    ...
    public function addOrder(Request $request)
    {
        ...
        try {
            ...
            $Client->save();
            orderTracking::insertOrderTracking($order->id, $order->status, "New Order is add by " . auth()->user()->fullname . " on  " . $order->created_at, auth()->user()->fullname, auth()->user()->id, "تمت اضافه عنصر بواسطه " . auth()->user()->fullname . 'في' . $order->created_at);
            if ($order) {
    ...
    public function addOrderWithApiKey(Request $request)
    {
        ...
        try {
            ...
            // return   $Client_id = ClientsTokens::where('api_key', $apikey)->where('api_secret_token', $request->api_secret_token)->first();
            // $clientsTokens->Client->fullname = 'jksa altignai';
            // $clientsTokens->Client->save();
            ...
            $Client->save();
            // dd($Client);
            orderTracking::insertOrderTracking($order->id, $order->status, "New Order is add by " . $Client->fullname . " on  " . $order->created_at, $Client->fullname, $Client->id, "تمت اضافه عنصر بواسطه " . $Client->fullname . 'في' . $order->created_at);
            if ($order) {
                return response([
                    'status' => true,
                    'message' => 'Order Created successfully!',
