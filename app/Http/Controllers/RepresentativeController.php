    ...
    public function acceptOrder(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($order, $validatedData) {
                ...
                $note = "order has been accepted by representative ( $representative )";
                orderTracking::insertOrderTracking($validatedData['order_id'], "pickup", $note, 'representative', auth()->user()->id);
            });
    ...
    public function deliverOrder(Request $request)
    {
        ...
        try {
            DB::transaction(function () use ($validatedData) {
                ...
                //insert into order Tracking
                $representative = auth()->user()->fullname;
                $note = "order has been delivered by representative ( $representative )";
                $Client = Client::find($Order->client_id);
                ...
                //     $Order->id,
                // ));
                orderTracking::insertOrderTracking($validatedData['order_id'], "delivered", $note, 'representative', auth()->user()->id);
            });
            return response([
                "status" => true,
    ...
    public function transferOrder(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($validatedData) {
                ...
                orderTracking::insertOrderTracking($validatedData['order_id'], $status, $note, 'representative', auth()->user()->id);
            });
    ...
    public function getRepresentativeSubAreas()
    {
        try {
            $sub_areas = DB::table('representatives')
                ->join("representative_areas as ra", "ra.representative_id", "representatives.id")
                ->join("sub_areas", "sub_areas.area_id", "ra.area_id")
    ...
    public function returnOrder(Request $request)
    {
        ...
        try {
            DB::transaction(function () use ($validatedData) {
                ...
                orderTracking::insertOrderTracking($validatedData['order_id'], "returned", $note, 'representative', auth()->user()->id);
            });
    ...
    public function inProgressOrder(Request $request)
    {
        ...
        try {
            DB::transaction(function () use ($validatedData) {
                ...
                //insert into order Tracking
                $representative = auth()->user()->fullname;
                $note = "order in progress with representative ( $representative )";
                orderTracking::insertOrderTracking($validatedData['order_id'], "inProgress", $note, 'representative', auth()->user()->id);
            });
