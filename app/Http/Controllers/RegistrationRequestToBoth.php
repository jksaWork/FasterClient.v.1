...
class RegistrationRequestToBoth extends Controller
{
    public function getUnAprrovedclient(){
        $Clients = Client::with('Files')->withoutGlobalScope(new ApprovedScope)->where('is_approved' , 0)->paginate();
        // return $Clients[0];
        return  view('clients.aprrove' ,compact('Clients'));
    }

    public function getUnAprrovedrepresentatives(){
        $Representatives = Representative::withoutGlobalScope(new ApprovedScope)->where('is_approved' , 0)->paginate();
