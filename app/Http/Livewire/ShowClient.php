...
class ShowClient extends Component
{
    ...
    public $fullname,
     $address, $email, $password, $passwordConfirm, $is_active, $sub_area_id,
    $phone, $discount_rate, $account_balance, $identify_image , $area_id,  $civil_registry ,
    $bank_account_owner , $bank_account_number , $iban_number, $commrical_record_image ,
    $bank_account_image , $client_type , $bank;
    public $client_id , $APIKey , $APISecretKey , $public_key_id;
    public $updateMode = false;
    public $serviec_1, $serviec_2, $serviec_3, $serviec_4, $serviec_5, $is_has_custom_price, $ClientServices = [];
    public $searchTerm;
    ...
    public function store()
    {
            ...
            if ($this->is_has_custom_price) {
                ...
                $InsertData  = [
                    ['client_id' => $Client->id, 'service_id' => 1,   'price' => $this->serviec_1],
                    ['client_id' => $Client->id, 'service_id' => 2,  'price' => $this->serviec_2],
                    ['client_id' => $Client->id, 'service_id' => 3, 'price' => $this->serviec_3],
                    ['client_id' => $Client->id, 'service_id' => 4,   'price' => $this->serviec_4],
                    ['client_id' => $Client->id, 'service_id' => 5,  'price' => $this->serviec_5],
                ];
                // dd($InsertData);
                $ServicePrice = ClientServicePrice::insert($InsertData);
                $this->serviec_1 = $this->serviec_2 = $this->serviec_3 = $this->serviec_4 = $this->serviec_5  = $this->is_has_custom_price =  '';
            }
    ...
    // public function DonePayment($status, $modalId)
    // {
    //     if ($status == "success") {
    //         session()->flash("success", __('translation.fees.transferred.successfully'));
    //         $this->render();
    //     } else if ($status == "error") {
    //         session()->flash("error", __('translation.error.exception'));
    //     } else if ($status == "amountError") {
    //         session()->flash("error", __('translation.error.amountError'));
    //     }
    // }
...
'البنك العربي الوطني'
        ...
        $data =  (Client::where('fullname', 'like', $searchTerm)->where('is_approved' , 1)->with('ServicePrice' , 'Area')->orderBy('id', 'desc')->paginate(10));
        if($this->area_id){
