...
class ShowRepresentativeAccountStatement extends Component
{
    ...
    private function getFilteredRepresentativeAccountStatement()
    {
        ...
        if ($this->representative_id) {
            $query = DB::table('representative_account_statement_report')
                // ->join('orders', 'orders.id', "=", "representative_orders_percentages.order_id")
                ->leftJoin('transactions', 'transactions.id', "=", "representative_account_statement_report.transaction_id")
                ->when($this->representative_id, function ($query) {
                    $query->where('representative_account_statement_report.representative_id', $this->representative_id);
                })
                ->when($this->from_date, function ($query) {
                    $query->where('representative_account_statement_report.Date', ">=", $this->from_date . " 00:00:00");
                })
                ->when($this->to_date, function ($query) {
                    $query->where('representative_account_statement_report.Date', "<=", $this->to_date . " 23:59:59");
                })
                ->select(['representative_account_statement_report.*', 'transactions.trans_sn'])->get();
            // dd($query);
